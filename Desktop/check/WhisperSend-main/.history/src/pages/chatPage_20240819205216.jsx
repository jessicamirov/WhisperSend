...
import DecryptModal from "../components/DecryptModal"; // נניח שהמודל נמצא בקובץ הזה
import { PeerIdContext } from "../components/peerIdContext";

export default function ChatPage({ connectPeerId }) {
    const { messages, setMessages, myWallet, recipientPeerId } = useContext(PeerIdContext);
        ...
        <ChatLayout connectPeerId={connectPeerId}>
            <div className="flex">
                ...
                <div className="w-3/4 pl-4">
                    <ChatMessages />
                    <MessageInput openDecryptModal={openDecryptModal} />
                </div>
            </div>
            <ToastContainer />
            {isDecryptModalOpen && (
