...
export const encryptFile = (
    ...
    return {
        nonce: Buffer.from(nonce).toString("hex"),
        encrypted: Buffer.from(encrypted).toString("hex"),
        fileType: file.type // שמירת סוג הקובץ המקורי
    };
};
...
export const decryptFile = (
    ...
    try {
        const encryptedObj = JSON.parse(encryptedMessage);
        const { nonce, encrypted, fileType } = encryptedObj;
        const sharedSecret = getSharedSecret(
            senderPublicKey,
            recipientPrivateKey
        );
        ...
        if (decrypted) {
            ...
            return new Blob([Buffer.from(decrypted)], { type: fileType || "application/octet-stream" });
        } else {
            console.error(
                "Decryption failed. Check keys, nonce, and encrypted values."
            );
