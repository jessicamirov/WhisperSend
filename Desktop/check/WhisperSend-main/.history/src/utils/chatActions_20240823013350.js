...
export const handleReceiveFile = async (
    ...
    try {
        // בדיקה אם data.data מוגדר ולא ריק
        if (data.data && typeof data.data === "string" && data.data.length % 2 === 0 && /^[0-9a-f]+$/i.test(data.data)) {
            ...
            if (!nonce || !encryptedData || !fileType) {
                throw new Error("Invalid data received: nonce, encryptedData, or fileType is missing.");
            }
            ...
            if (shouldDecrypt) {
                ...
                if (decryptedBlob) {
                    ...
                    fileName = fileName.replace(".txt", `.${fileType.split("/")[1]}`); // החזרת הסיומת המקורית לפי סוג הקובץ
        ...
        console.error("Error in handleReceiveFile:", error);
    }
};
