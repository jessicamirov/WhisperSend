        ...
        try {
            ...
            reader.onload = () => {
                try {
                    ...
                    const nonceBuffer = Uint8Array.from(Buffer.from(nonce, "hex"));
                    const encryptedBuffer = Uint8Array.from(Buffer.from(encrypted, "hex"));
                    ...
                    const decryptedBuffer = nacl.box.open.after(encryptedBuffer, nonceBuffer, sharedSecret);
                    if (!decryptedBuffer) {
                    ...
                    // יצירת Blob עם סוג הקובץ הנכון
                    const blob = new Blob([decryptedBuffer], {
                        type: fileType || "application/octet-stream",
                    });
    
                    // חילוץ הסיומת מתוך ה-MIME type, אם קיים
                    const extension = fileType ? fileType.split("/").pop() : "bin";
    
                    // שמירה בשם הנכון
                    const originalFileName = `decryptedFile.${extension}`;
                    const link = document.createElement("a");
                    link.href = URL.createObjectURL(blob);
                    link.download = originalFileName;
                    link.click();
