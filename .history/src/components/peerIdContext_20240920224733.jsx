...
export const PeerIdProvider = ({ children }) => {
    ...
    const disconnect = (initiatedByUser = false) => {
        localInitiatedDisconnect = initiatedByUser// Set local state for initiated disconnection
        if (initiatedByUser) {
            setInitiatedDisconnect(true)// Update state if disconnect is initiated by user
        }

        safelyCloseConnection()// Close the connection safely
        performDisconnect()// Reset state and navigate away from chat
    }

    /**
    * Handles the disconnection process, resetting the chat state and navigating the user back to "Share Securely".
    */
...
/**
 * Manages the scenario when the peer disconnects, either by rejection or unintentional disconnection.
 * If the disconnection is due to rejection or initiated by the user, it calls `performDisconnect`.
 * Otherwise, it opens a confirmation modal asking the user if they want to exit the chat.
 *  If confirmed, "performDisconnect" is called. Otherwise, the user remains in the disabled chat.
 */
...
/**
 * Clears the messages and navigates the user back to the "Share Securely" page.
 * - Routes the user back to the main secure sharing page.
 */
    const handleExit = () => {
        setMessages([])
        setMessage("")
    ...
    // Handles messages and files received from the peer.
    const handleData = (data, senderPeerId) => {

        try {
            if (data instanceof ArrayBuffer || data instanceof Blob) {
                // If the data is a file, handle file reception
                handleReceiveFile(
                    messages,
                    setMessages,
                    myWallet.privateKey,
                ...
                const parsedData = JSON.parse(data)// Parse the message data
