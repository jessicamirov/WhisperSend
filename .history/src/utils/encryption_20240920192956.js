 ...
 * 
 * This function takes a plain text message and encrypts it using the recipient's public key 
 * and the sender's private key. The encryption process follows these steps:
 * 1. Derive a shared secret using Elliptic Curve Diffie-Hellman (ECDH) with the sender's private key and the recipient's public key.
 * 2. Generate a nonce (random number used once) to ensure message uniqueness.
 * 3. Encrypt the text message using NaCl's `box.after` function with the shared secret and nonce.
 * 4. Return both the encrypted message and the nonce (both encoded in hex) for later decryption.
 */
