...
export const handleReceiveFile = async (
    ...
    if (
        ...
        if (!shouldDecrypt) {
            ...
            if (decryptedBlob) {
                // שימוש בפונקציה להורדת הקובץ עם סיומת נכונה
                const blob = new Blob([decryptedBlob], {
                    type: fileTypeFinal || "application/octet-stream", // שימוש בסוג הקובץ או ברירת מחדל
                });

                // שינוי הסיומת חזרה לשם מקורי ללא .json
                const originalFileName = fileName.replace(".json", "");
                
                // שמירה והורדה של הקובץ המפוענח
                const link = document.createElement("a");
                link.href = URL.createObjectURL(blob);
                link.download = `decrypted${globalMessageCount}.${fileTypeFinal.split("/").pop()}`;
                link.click();
